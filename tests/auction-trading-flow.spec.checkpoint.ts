...
async function submitBid(page: Page, role: "buyer" | "seller"): Promise<void> {
  const actionButtonName = role === "buyer" ? "Comprar" : "Vender";
  const price = role === "buyer"
...
async function exploreTradingScreen(page: Page, role: "buyer" | "seller"): Promise<void> {
  await submitBid(page, role);
}
...
test.describe("Acceso a ruedas - Ronda 1 automatizada", () => {
  test("seller: entra a rueda y envía postura de venta", async ({ page }) => {
    ...
    await exploreTradingScreen(page, "seller");
  });
  ...
  test("buyer: entra a rueda y envía postura de compra", async ({ page }) => {
    ...
    await exploreTradingScreen(page, "buyer");
  });
});
